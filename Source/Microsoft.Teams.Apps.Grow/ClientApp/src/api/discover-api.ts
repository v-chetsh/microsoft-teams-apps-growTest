...
/**
* Filter data based on title and tags.
...
/**
* Filter data based on title and tags for a team.
* @param searchText Search text typed by user
...
export const joinProject = async (projectContent : any): Promise<any> => {
    let url = baseAxiosUrl + `/project-workflow/join-project/${projectContent.projectId}`;
...
export const closeProject = async (participantDetails: any): Promise<any> => {
    let url = baseAxiosUrl + `/project-workflow/close-project/${participantDetails.projectId}`;
    return await axios.patch(url, participantDetails);
}
